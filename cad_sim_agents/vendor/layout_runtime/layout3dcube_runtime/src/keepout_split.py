...
import numpy as np
from src.schema import AABB, Envelope
...
def build_bins(envelope: AABB, keepouts: List[AABB], min_edge_threshold: float = 5.0) -> List[AABB]:
    ...
    print(f"  子容器总体积: {sum(b.volume() for b in bins_filtered):.0f} mm³")
    
    return bins_filtered


def build_envelope(cfg: dict, parts) -> Envelope:
...
def create_keepout_aabbs(cfg: dict) -> List[AABB]:
    """根据配置创建禁区AABB列表"""
    keepouts = []
