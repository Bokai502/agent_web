...
from src.schema import AABB, Part
...
def _single_run_pack(
    ...
    for (bin_idx, face_id) in face_tasks:
        ...
        print(f"The face {face_id} of bin {bin_idx} placed_face: {[pp.placed.id for pp in placed_face]}")
...
def multistart_pack(
    ...
    return best_placed_global, best_unplaced_global
