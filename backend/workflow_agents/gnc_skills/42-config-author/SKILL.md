...
description: Generate or modify valid 42 configuration files from approved scenario facts and audited capability decisions, using existing workspace input templates whenever practical.
---
...
Use this skill only after scenario facts are stable and 42 support has already been audited. The job is to create the minimum valid 42 workspace configuration that reflects the approved scenario without inventing unsupported capabilities.
...
- a new 42 configuration set
- modifications to existing `InOut` configuration files
...
- existing workspace configuration files to patch rather than recreate

## Required Local Context

Read `skills/42-config-author/references/repo-sources.md` first.
...
For ordinary satellite scenarios, use the current workspace input template as a complete configuration artifact bundle and keep the mutable set narrow by default. Copy or reuse the ancillary template files unchanged, then fully author the core configuration files:
...
### 2. Fully author the core runnable configuration first

Prioritize:

- valid syntax
- internally consistent cross-file references
- a runnable configuration structure
...
Produce under `workspace_dir/AIGNC_Workflow/04_config/`:
...
5. Does `workspace_dir/AIGNC_Workflow/04_config/generated_config_manifest.json` list files created, files modified, templates reused, core-file field decisions, assumptions applied, and retained defaults?
...
Do not write final runtime configuration files directly to `workspace_dir/00_inputs/Config/`; that directory is populated only after `42-config-validator` passes.
